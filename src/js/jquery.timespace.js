...
$.fn.timespace = function (options, callback) {
    ...
    this.Timespace = $.data(this).Timespace;
...
Timespace = {
	initValues: function(){
		...
		this.transition= -1;
		this.transitionEase= null;
		this.viewData= null;
		this.selectedEventArticle = true;

		// Elements
        this.containerWrapper = '<div class="jqTimepsaceContainerWrapper"></div>';
		this.container= '<div class="jqTimepsaceContainer"></div>';
		this.error= '<div class="jqTimespaceErrors"></div>';
		this.display= '<article class="jqTimespaceDisplay"></article>';
		this.displayTitle= '<h1></h1>';
		this.displayTimeDiv= '<div class="jqTimespaceDisplayTime"></div>';
		this.displayTime= '<p></p>';
		this.displayBody= '<section></section>';
		this.displayLeft= '<div class="jqTimespaceDisplayLeft" title="Previous Event"></div>';
		this.displayRight= '<div class="jqTimespaceDisplayRight" title="Next Event"></div>';
		this.navLeft= '<div class="jqTimespaceLeft" title="Move Left"><span>&#x027E8;</span></div>';
		this.navRight= '<div class="jqTimespaceRight" title="Move Right"><span>&#x027E9;</span></div>';
		this.tableContainer= '<div class="jqTimespaceTableContainer"></div>';
		this.timeTableLine= '<div class="jqTimespaceLine"></div>';
		this.timeTable= '<table></table>';
		this.timeTableHead= '<thead><tr></tr></thead>';
		this.timeTableBody= '<tbody></tbody>';
		this.timeMarkers= null;
		this.timeEvents= null;
		this.wideHeadings= null;
		this.curWideHeading= null;
		this.curEvent= null;
		this.tooltipOptions = {
            position: 'top',
            html: true,
            template: '<div class="tooltip timespace" role="tooltip">' +
        '<div class="tooltip-arrow"></div>' +
        '<div class="tooltip-inner {{className}}"></div>' +
        '</div>'
        };
		this.rowLabel = '<div class="jqTimespaceTableRowLabel"></div>';
	},
    ...
    load: function (target, options) {
        ...
        // Setup Base Elements
        this.containerWrapper = $(this.containerWrapper).appendTo(target)
            .on('resize.jqTimespace', this.updateDynamicData.bind(this));
        this.container = $(this.container).appendTo(this.containerWrapper);
        ...
        this.selectedEventArticle = opts.hasOwnProperty('selectedEventArticle')? opts.selectedEventArticle: true;
        ...
        if (this.timeEvents.length > 0 && opts.selectedEvent >= 0) {
            if (this.selectedEventArticle) {
                this.timeEvents.eq(opts.selectedEvent).trigger('mouseup', [true]);
            }
            else {
                this.display.hide();
                this.scrollToEvent(this.timeEvents.eq(opts.selectedEvent));
            }
	...
	updateEvents: function(events){
        ...
        prevOptions.data.events = events;
        ...
        // Setup Base Elements
		$(this.target).empty();
        this.containerWrapper = $(this.containerWrapper).appendTo(this.target)
            .on('resize.jqTimespace', this.updateDynamicData.bind(this));
        this.container = $(this.container).appendTo(this.containerWrapper);
        this.error = $(this.error).appendTo(this.container);
        this.tableContainer = $(this.tableContainer)
    ...
    buildTimeHeadings: function () {
        ...
        if (this.data.headings) {
            this.data.headings.forEach((v, i, a) => {
                ...
                title = utility.sanitize(v.markerContent? v.markerContent: v.title);
    ...
    buildTimeMarkers: function () {
    	...
    	if(opts.data && opts.data.events){
    		noOfRows = Object.keys(opts.data.events).length;
		}
        ...
        for (let i = 0; i < noOfRows; i += 1) {
            this.timeMarkers = this.timeMarkers.add(row.clone());
        }
    ...
    buildTimeEvents: function () {
        ...
        if (this.data.events) {
        	var self = this;
        	Object.keys(self.data.events).forEach((rowLabel, rowIndex) => {
        		var rowEvents = $();

				this.data.events[rowLabel].forEach((v, i) => {

					const start = parseFloat(v.start) || null,
					end = parseFloat(v.end) || null,
					title = v.markerContent? v.markerContent: v.title,
					...
					evtClass = (!utility.isEmpty(v.class))
						? ` class="${utility.sanitize(v.class)} jqTimespaceEventMarker"` : 'no-class jqTimespaceEventMarker',
					eventCallback = (utility.isEmpty(v.callback))
						? $.noop : v.callback.bind(this.API),
					tooltipContent = v.tooltipContent? v.tooltipContent: null,
					tooltipClassName = v.tooltipClass || '';
					...
					const rounded = utility.roundToIncrement('floor', opts.markerIncrement, start),
						...
						eventElem = $(`<div${evtClass}></div>`).prependTo(event);
					    eventElem.html(title);
					...
					if (index >= 0) {
						...
						var prevSib = event.prev();
						// if(prevSib.length > 0) pos -=
						event.css('left', pos + 'px').appendTo($(this.timeMarkers[rowIndex]).children('td')[index]);
					...
					// })())
                        ...
                        rowEvents = rowEvents.add($('<td></td>').append($(eventElem).clone()));
                        ...
                        // if(tooltipContent) {
                        //     eventElem.attr('title', tooltipContent);
                        //     var options = Object.assign({}, this.tooltipOptions);
                        //     options.template = options.template.replace('{{className}}', tooltipClassName);
                        //     $(eventElem).tooltip(options);
                        // }
				...
				events = events.add($('<tr class="' + rowLabel + '"></tr>').append(rowEvents));
			});
        ...
        var timeEvents = this.timeMarkers.find('p');
        if(timeEvents.length === 0) timeEvents = this.timeMarkers.find('.jqTimespaceEventMarker');

        timeEvents.sort(function (a, b) {
            var eventA =parseInt($(a).parents('td').position().left) + parseInt($(a).position().left);
            var eventB =parseInt($(b).parents('td').position().left) + parseInt($(b).position().left);
        ...
        this.timeEvents.each(function () {
            ...
            if (!elem.data('noDetails')) {
                ...
                && Math.abs(ts.viewData.shiftOriginY - ts.getTableBodyPosition()) < 10) {
                    ...
                    if(this.selectedEventArticle) ts.displayEvent(elem, preventScroll);
                }
    ...
    timeShiftPos: function (plane) {
        ...
        if ((this[shiftDir] === '<' && this[shiftPos] >= -this.viewData[tableOffset])
            ...
            if(this.overflowCount === undefined || this.overflowCount > 0){
                this.onRangeOverflow('right');
                this.overflowCount= undefined;
            } else if(this.overflowCount === undefined) {
                this.overflowCount = 0;
            } else {
    ...
    getTableBodyPosition: function () {
        return this.timeTableBody.css('top') !== 'auto'? parseFloat(this.timeTableBody.css('top')): 0;
    },
    ...
    onRangeOverflow: function(direction){
        console.log('overFlow', direction);
    },

    buildRowLabels: function(){
        var labels = Object.keys(this.data.events);

        var bodyOffsetTop = $(this.timeTableBody).offset().top || 0;
        var self = this;
        ...
        $(this.timeTableBody).find('tr').each(function(index, row){
            var rowOffsetTop = $(row).offset().top || 0;
            var offsetTop = rowOffsetTop - bodyOffsetTop;
            var td = $(row).find('td');
            var extra = (parseInt(td.css('margin-top')) || 0) + (parseInt(td.css('padding-top')) || 0);

            var rowLabel = $(self.rowLabel);
            rowLabel.text(uToc(labels[index]));
            rowLabel.css('top', offsetTop + extra + 'px');
            rowLabel.prependTo(self.tableContainer);
        });
    ...
    scrollToEvent: function(elem) {
        var top = elem.offset().top;
        var center = parseInt($(this.tableContainer).css('width')) / 2;

        this.navigate([
            $(this.timeTableLine).position().left - (elem.parents('td').position().left - center) - elem.parents('div').position().left,
